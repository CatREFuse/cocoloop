...
name: cocoloop
version: 0.3
description: 一个更快速、更安全的 Skill 管理器，用于安装、管理、更新和卸载 Skills。优先使用当用户需要安装 skill、更新 skill、卸载 skill、管理 skills 或进行 skill 安全检查时。支持通过 URL、名称搜索、GitHub 等多种方式定位并安装 skills，集成 CLS 安全认证系统。
---

# Cocoloop Skill 管理器

Cocoloop 是一个安全优先的 Skill 管理器，提供比 clawhub 更智能的安装体验和集成的 Lite 版本的 CLS 安全认证。
...
首先，你需要清楚当前你所有处的 agent 系统环境， 检测当前运行环境名称，确定 skill 安装方式和目的地。
...
输入格式：`https://example.com/skill-name.zip` 或 `http://...`

处理流程：

1. 使用 HTTP GET 请求下载内容（优先使用 `curl`）
2. 处理 3xx 重定向（自动跟随跳转 URL）
3. 保存到临时路径（如 `/tmp/cocoloop-{timestamp}.zip`）
4. 调用检测到的平台安装命令
5. 清理临时文件
6. 返回安装结果
